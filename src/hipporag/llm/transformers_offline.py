...
class TransformersOffline:
    ...
    def batch_infer(
        ...
        if len(messages_list) > 1:
            logger.info(
                f"Calling Transformers offline, # of messages {len(messages_list)}, using batchsize = 4"
            )
        ...
        if json_template is not None:
            ...
            for i in range(0, len(all_prompt_texts), 4):
                transformers_output = generator(
                    all_prompt_texts[i : i + 4], max_tokens=max_tokens
                )
                transformers_outputs.extend(transformers_output)
        else:
            transformers_outputs = []
            for i in range(0, len(all_prompt_texts), 4):
                transformers_output = self.model.generate(
                    all_prompt_texts[i : i + 4], max_tokens=max_tokens
                )
                transformers_outputs.extend(transformers_output)
        all_responses = [
            completion.model_dump_json() for completion in transformers_outputs
        ]
        all_prompt_tokens = [
            len(self.tokenizer.encode(prompt)) for prompt in all_prompt_texts
        ]
        all_completion_tokens = [
            len(self.tokenizer.encode(response)) for response in all_responses
        ]
