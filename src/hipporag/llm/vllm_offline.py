...
class VLLMOffline:
    ...
    def infer(self, messages: List[TextChatMessage], max_tokens=2048):
        ...
        prompt_ids = convert_text_chat_messages_to_input_ids(
    ...
    def batch_infer(
        ...
        if json_template is not None:
            ...
            guided = GuidedDecodingRequest(
                guided_json=PROMPT_JSON_TEMPLATE[json_template]
            )

        all_prompt_ids = [
            convert_text_chat_messages_to_input_ids(messages, self.tokenizer)
        ...
        all_prompt_tokens = [
            len(completion.prompt_token_ids) for completion in vllm_output
        ]
        all_completion_tokens = [
            len(completion.outputs[0].token_ids) for completion in vllm_output
        ]
